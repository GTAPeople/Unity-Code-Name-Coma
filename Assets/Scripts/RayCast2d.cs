...
public class RayCast2d : MonoBehaviour {
	...
	void Update () {
		...
		for (int i = 1; i < RaysToShoot + 1; i++)
		{
			...
			RaycastHit2D hit = Physics2D.Linecast(SightStart.position, endPoint, 1 << LayerMask.NameToLayer("Ground"));
	...
	void RayCasting()
	{
		...
		spooted = Physics2D.Linecast(SightStart.position, SightEnd.position, 1 << LayerMask.NameToLayer("Ground"));
		spooted2 = Physics2D.Linecast(SightStart.position, SightEnd2.position, 1 << LayerMask.NameToLayer("Ground"));
		spooted3 = Physics2D.Linecast(SightStart.position, SightEnd3.position, 1 << LayerMask.NameToLayer("Ground"));
	}
