...
public class PlayerBall2 : MonoBehaviour {
	...
	void Update () 
	{
		...
		{
			RaycastHit2D hitUp = Physics2D.Linecast(transform.position, up, 1 << LayerMask.NameToLayer("Enemy"));
			RaycastHit2D hitDown = Physics2D.Linecast(transform.position, down, 1 << LayerMask.NameToLayer("Enemy"));
			RaycastHit2D hitLeft = Physics2D.Linecast(transform.position, left, 1 << LayerMask.NameToLayer("Enemy"));
			RaycastHit2D hitRight = Physics2D.Linecast(transform.position, right, 1 << LayerMask.NameToLayer("Enemy"));
			...
			if(hitRight.point != new Vector2(0,0))
			{
				...
				if(downCollition)
				{
					...
					for (int i = (int)anguloGiro; i < 180; i+= aumento)
					{
						...
						RaycastHit2D hitUptmp = Physics2D.Linecast(transform.position, uptmp, 1 << LayerMask.NameToLayer("Enemy"));
